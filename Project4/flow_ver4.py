...
from Project4.ground import extract_groundintensity
...
def extrapolate_flow(dps_images:np.ndarray, V:np.ndarray, earth_image, timesDay, times, mask, minutes_after:int=15, batch_size:int=0, objects=[], show:bool=False) -> list:
    """
    ...
    for i, t in enumerate(objects):
        ...
        # Fill nan values from the earth image
        # Example usage:
        # Assuming interpolate_image and mask are defined, and interpolate_image contains NaN values
        fill_image_with_max(extrapolate_image, mask, use_global_max=True)

        # fill_image(extrapolate_image, mask, value = 40)
...
if __name__ == "__main__":
    ...
    for target in target_days:
        ...
        interpolate_flow(dps_images, V, earth_image, timesDay, times, mask, objects=objects, show=True, n=15)
