...
def interpolate_flow(dps_images:np.ndarray, V:np.ndarray, timesDay, times, mask, batch_size:int=0, n:int=1, objects=[], show:bool=True):
    ...
    for i, t in enumerate(objects):
        ...
        for k, img in enumerate(interpolate_images):
            ...
            # if show:
            #     if k == 0:
            #         plt.imshow(original_image*mask, cmap="viridis")
            #         plt.title(f"202403{timesDay[t]}_" + times[t])
            #         plt.pause(0.5)
            #         plt.clf()
        ...
        axs[0].imshow(original_image*mask, cmap="viridis")
        axs[0].set_title(f"Original Image 202403{timesDay[t]}_{timestamps[0]}")
        axs[1].imshow(interpolate_images[0]*mask, cmap="viridis")
        axs[1].set_title(f"Interpolated Image 202403{timesDay[t]}_{timestamps[1]}")
        axs[2].imshow(interpolate_images[1]*mask, cmap="viridis")
        axs[2].set_title(f"Interpolated Image 202403{timesDay[t]}_{timestamps[2]}")#, MSE: {MSE(extrapolate_images[i], V[:,:,t + 1], mask):.2f}, Baseline MSE: {MSE(V[:,:,t], V[:,:,t + 1], mask):.2f}")
        plt.tight_layout()
        plt.savefig(f"interpolation.svg")
...
def extrapolate_flow(dps_images:np.ndarray, V:np.ndarray, timesDay, times, mask, minutes_after:int=15, batch_size:int=0, objects=[], show:bool=False) -> list:
    ...
    for i, t in enumerate(objects):
        ...
        if show:
            ...
            axs[1].set_title(f"True Image 202403{timesDay[t+1]}_{times[t+1]}")
            ...
            plt.tight_layout()
            plt.savefig(f"extrapolation.svg")
...
if __name__ == "__main__":
    ...
    for target in target_days:
        ...
        objects=range(25,26)  # interpolation
        # objects=range(10,11)        # extrapolation
